...
const PostDetail: React.FC = () => {
  ...
  // Determine the image URL for sharing
  const imageUrl = post.featuredImageUrl
    ? /^[A-Za-z0-9+/=]+$/.test(post.featuredImageUrl)
      ? `${window.location.origin}/api/image/${post.slug}` // Assuming an endpoint to serve base64 as an image
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="twitter:description" content={post.excerpt || post.content.substring(0, 150)} />
